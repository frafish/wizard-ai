...
class WizardAI_RAG_Embeddings_Cron {
    ...
    public function __construct() {
        $this->provider = get_option('wbai_rag_embedding_provider', '');
    ...
    private function init_db() {
        ...
        $this->db_dir = $upload_dir['basedir'] . '/wbai';
    ...
    public function run() {
        ...
        if (get_option('wbai_rag_sync_contents', 1) == 1 || get_option('wbai_rag_sync_products', 1) == 1) {
            $this->process_posts();
        }
        if (get_option('wbai_rag_sync_terms', 1) == 1) {
            $this->process_terms();
        }
        if (get_option('wbai_rag_sync_settings', 1) == 1) {
            $this->process_settings();
        }
        if (get_option('wbai_rag_sync_plugins', 1) == 1) {
