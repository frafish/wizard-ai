...
abstract class Abstract_Ability extends WP_Ability {
	...
	protected function set_provider_model_preference( \WP_AI_Client_Prompt_Builder $prompt_builder, string $feature_class, array $fallback_models = array() ): \WP_AI_Client_Prompt_Builder {
        ...
        // --- INJECT WIZARD AI SEO SETTINGS ---
        $is_image_generation = is_a($this, '\WordPress\AI\Abilities\Image\Generate_Image') || is_a($this, '\WordPress\AI\Abilities\Image\Generate_Image_Prompt');
        if ($is_image_generation) {
            $model_setting = get_option('wai_seo_preferred_model', '');
        } else {
            $model_setting = get_option('wai_seo_text_model', '');
        }

        if (!empty($model_setting)) {
            $parts = explode('|', $model_setting);
            if (count($parts) === 2) {
                try {
                    $prompt_builder->using_model(
                        AiClient::defaultRegistry()->getProviderModel($parts[0], $parts[1])
                    );
                    return $prompt_builder;
                } catch (\Exception $e) {
                    // Fallback to default behavior if provider/model is not found
                }
            }
        }
        // -------------------------------------
