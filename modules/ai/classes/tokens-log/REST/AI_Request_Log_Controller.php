...
namespace WizardAi\Modules\Ai\Classes\TokensLog\REST;
...
class AI_Request_Log_Controller extends WP_REST_Controller {
	...
	public function get_log( WP_REST_Request $request ) {
		...
		if ( ! $log ) {
			return new WP_Error(
				'wpai_log_not_found',
				__( 'Log entry not found.', 'wizard-ai' ),
				array( 'status' => 404 )
			);
		}
		...
		return rest_ensure_response(
			array(
				...
				'message' => sprintf(
					...
					__( 'Successfully purged %d log entries.', 'wizard-ai' ),
					$deleted
				),
			)
		);
	...
	public function get_collection_params(): array {
		return array(
			'type'             => array(
				'description' => __( 'Filter by log type.', 'wizard-ai' ),
				'type'        => 'string',
				'enum'        => array( '', 'ai_client', 'mcp_tool', 'ability' ),
				'default'     => '',
			),
			'status'           => array(
				'description' => __( 'Filter by status.', 'wizard-ai' ),
				'type'        => 'string',
				'enum'        => array( '', 'success', 'error', 'timeout' ),
				'default'     => '',
			),
			'provider'         => array(
				'description' => __( 'Filter by AI provider.', 'wizard-ai' ),
				'type'        => 'string',
				'default'     => '',
			),
			'user_id'          => array(
				'description' => __( 'Filter by user ID.', 'wizard-ai' ),
				'type'        => 'integer',
				'default'     => 0,
			),
			'date_from'        => array(
				'description' => __( 'Filter logs from this date (YYYY-MM-DD HH:MM:SS).', 'wizard-ai' ),
				'type'        => 'string',
				'format'      => 'date-time',
			),
			'date_to'          => array(
				'description' => __( 'Filter logs until this date (YYYY-MM-DD HH:MM:SS).', 'wizard-ai' ),
				'type'        => 'string',
				'format'      => 'date-time',
			),
			'search'           => array(
				'description' => __( 'Search in operations, request previews, response previews, and error messages.', 'wizard-ai' ),
				'type'        => 'string',
				'default'     => '',
			),
			'tokens_filter'    => array(
				'description' => __( 'Filter by tokens: "gt:N", "lt:N", or "none".', 'wizard-ai' ),
				'type'        => 'string',
				'default'     => '',
			),
			'page'             => array(
				'description' => __( 'Current page of the collection.', 'wizard-ai' ),
				'type'        => 'integer',
				'default'     => 1,
				'minimum'     => 1,
			),
			'per_page'         => array(
				'description' => __( 'Maximum number of items per page.', 'wizard-ai' ),
				'type'        => 'integer',
				'default'     => 25,
				'minimum'     => 1,
				'maximum'     => 100,
			...
			'orderby'          => array(
				'description' => __( 'Sort collection by attribute.', 'wizard-ai' ),
				'type'        => 'string',
				'enum'        => array( 'timestamp', 'type', 'operation', 'duration_ms', 'tokens_total', 'status' ),
				'default'     => 'timestamp',
			),
			'order'            => array(
				'description' => __( 'Order sort attribute ascending or descending.', 'wizard-ai' ),
				'type'        => 'string',
				'enum'        => array( 'ASC', 'DESC' ),
				'default'     => 'DESC',
			),
			'cursor_id'        => array(
				'description' => __( 'Cursor ID for cursor-based pagination (use with cursor_timestamp).', 'wizard-ai' ),
				'type'        => 'integer',
				'minimum'     => 1,
			),
			'cursor_timestamp' => array(
				'description' => __( 'Cursor timestamp for cursor-based pagination (use with cursor_id).', 'wizard-ai' ),
				'type'        => 'string',
				'format'      => 'date-time',
			),
		);
