...
namespace WizardAi\Modules\Ai\Classes\TokensLog;
/**
...
class AI_Request_Log_Page {
	...
	public function enqueue_assets(): void {
		...
		$dataviews_css = WIZARD_AI_PATH . 'modules/ai/assets/tokens-log/dataviews.css';
		if ( ! wp_styles()->query( 'wp-dataviews' ) && file_exists( $dataviews_css ) ) {
			wp_enqueue_style(
				'ai-dataviews',
				WIZARD_AI_URL . 'modules/ai/assets/tokens-log/dataviews.css',
				array(),
				(string) filemtime( $dataviews_css )
			);
		}

		$asset_file = WIZARD_AI_PATH . 'modules/ai/assets/tokens-log/ai-request-logs.asset.php';
		$asset      = file_exists( $asset_file ) ? require $asset_file : array( 'dependencies' => array(), 'version' => '1.0.0' );
		
		wp_enqueue_script(
			'ai_request_logs',
			WIZARD_AI_URL . 'modules/ai/assets/tokens-log/ai-request-logs.js',
			$asset['dependencies'],
			$asset['version'],
			true
		);
		...
		wp_enqueue_style(
			'ai_request_logs',
			WIZARD_AI_URL . 'modules/ai/assets/tokens-log/ai-request-logs.css',
			array( 'wp-components' ),
			$asset['version']
		);
