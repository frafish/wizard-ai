...
trait McpOauth {
  ...
  private $ACCESS_TOKEN_TTL = 3600;       // 1 hour
  private $REFRESH_TOKEN_TTL = 2592000;   // 30 days
  private $AUTH_CODE_TTL = 60;            // seconds
  public const NONCE_ACTION = 'wizard_ai_oauth_consent';

  
  
  private $oauth_namespace = 'wizard-ai/v1';
  private $logging = false;
  private $table_clients;
  private $table_tokens;

  public function init_mcp_oauth() {
    global $wpdb;
    
    
    $this->logging = false;
    $this->table_clients = $wpdb->prefix . 'wizard_ai_oauth_clients';
    $this->table_tokens = $wpdb->prefix . 'wizard_ai_oauth_tokens';

    $this->maybe_upgrade_db();

    add_action( 'rest_api_init', [ $this, 'register_oauth_routes' ] );
  ...
  private function protected_resource_metadata() {
    $issuer = rest_url( $this->oauth_namespace );
    return [
      'resource' => rest_url( $this->oauth_namespace . '/mcp' ),
  ...
  private function authorization_server_metadata() {
    ...
    return [
      'issuer' => $issuer,
      'authorization_endpoint' => rest_url( $this->oauth_namespace . '/oauth/authorize' ),
      'token_endpoint' => rest_url( $this->oauth_namespace . '/oauth/token' ),
      'registration_endpoint' => rest_url( $this->oauth_namespace . '/oauth/register' ),
      'revocation_endpoint' => rest_url( $this->oauth_namespace . '/oauth/revoke' ),
  ...
  public function reauth_for_authorize( $result ) {
    $uri = isset( $_SERVER['REQUEST_URI'] ) ? (string) $_SERVER['REQUEST_URI'] : '';
    if ( strpos( $uri, '/' . $this->oauth_namespace . '/oauth/authorize' ) === false ) {
      return $result;
    }
  ...
  private function maybe_upgrade_db() {
    if ( get_option( 'wizard_ai_oauth_db_version' ) === $this->DB_VERSION ) {
      return;
    }
    ...
    update_option( 'wizard_ai_oauth_db_version', $this->DB_VERSION );
  }
  #endregion
  ...
  public function register_oauth_routes() {
    ...
    register_rest_route( $this->oauth_namespace, '/.well-known/oauth-protected-resource', [
      'methods' => 'GET',
    ...
    register_rest_route( $this->oauth_namespace, '/.well-known/oauth-authorization-server', [
      'methods' => 'GET',
    ...
    register_rest_route( $this->oauth_namespace, '/oauth/register', [
      'methods' => 'POST',
    ...
    register_rest_route( $this->oauth_namespace, '/oauth/authorize', [
      'methods' => [ 'GET', 'POST' ],
    ...
    // Token endpoint
    register_rest_route( $this->oauth_namespace, '/oauth/token', [
      'methods' => 'POST',
    ...
    register_rest_route( $this->oauth_namespace, '/oauth/revoke', [
      'methods' => 'POST',
    ...
    register_rest_route( $this->oauth_namespace, '/oauth/apps', [
      'methods' => 'GET',
    ...
    register_rest_route( $this->oauth_namespace, '/oauth/apps/(?P<id>\d+)', [
      'methods' => 'DELETE',
  ...
  public function handle_authorize( WP_REST_Request $request ) {
    ...
    if ( !is_user_logged_in() ) {
      $current_url = rest_url( $this->oauth_namespace . '/oauth/authorize' );
  ...
  private function handle_authorize_submit( WP_REST_Request $request ) {
    ...
    set_transient( $this->auth_code_key( $code ), $code_data, $this->AUTH_CODE_TTL );
  ...
  private function auth_code_key( $code ) {
    return 'wizard_ai_oauth_code_' . hash( 'sha256', $code );
  }
  #endregion
  ...
  private function issue_token_pair( $client_id, $user_id, $scope ) {
    ...
    $wpdb->insert( $this->table_tokens, [
      ...
      'access_expires' => gmdate( 'Y-m-d H:i:s', $now + $this->ACCESS_TOKEN_TTL ),
      'refresh_expires' => gmdate( 'Y-m-d H:i:s', $now + $this->REFRESH_TOKEN_TTL ),
      'scope' => $scope,
  ...
  public function user_can_authorize( $user_id ) {
    ...
    return (bool) apply_filters( 'wizard_ai_mcp_oauth_user_can_authorize', $allowed, $user_id );
  }
  #endregion
  ...
  public function add_www_authenticate_header( $response, $server, $request ) {
    ...
    if ( strpos($route, '/' . $this->oauth_namespace . '/mcp') !== 0 ) {
      return $response;
    }
    ...
    $resource_metadata = rest_url( $this->oauth_namespace . '/.well-known/oauth-protected-resource' );
    $response->header(
      'WWW-Authenticate',
  ...
  private function render_consent_page( $client, $params, $user ) {
    ...
    $action_url = rest_url( $this->oauth_namespace . '/oauth/authorize' );
